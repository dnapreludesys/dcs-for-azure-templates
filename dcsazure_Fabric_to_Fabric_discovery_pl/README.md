# dcsazure_Fabric_to_Fabric_discovery_pl
## Delphix Compliance Services (DCS) for Azure - Fabric to Fabric Discovery Pipeline

This pipeline will perform automated sensitive data discovery on your Fabric warehouse.

### Prerequisites
1. Configure the hosted metadata database and associated Azure SQL service (version `V2025.01.15.0`+).
1. Configure the DCS for Azure REST service.
1. Configure the Fabric warehouse linked service.

### Importing
There are several linked services that will need to be selected in order to perform the profiling and data discovery of your Fbaric warehouse.

These linked services types are needed for the following steps:


`Fabric warehouse` (source) - Linked service associated with unmasked Fabric warehouse data. This will be used for the following
steps:
* dcsazure_Fabric_to_Fabric_discovery_source_ds (Fabric warehouse dataset)
* dcsazure_Fabric_to_Fabric_discovery_df/Source1MillRowDataSampling (dataFlow)
...
* dcsazure_Fabric_to_Fabric_discovery_metadata_ds (Azure SQL Database dataset),
* dcsazure_Fabric_to_Fabric_discovery_df/MetadataStoreRead (dataFlow),
* dcsazure_Fabric_to_Fabric_discovery_df/WriteToMetadataStore (dataFlow)

`REST` (DCS for Azure) - Linked service associated with calling DCS for Azure. This will be used for the following
  steps:
* dcsazure_Fabric_to_Fabric_discovery_df (dataFlow)
...
* Schema Discovery From AzureSQL
  * Query metadata from Azure SQL `information_schema` to identify tables and columns in the Fabric warehouse.
* Select Discovered Tables
  * After persisting the metadata to the metadata store, collect the list of discovered tables
* For Each Discovered Table
  * Call the `dcsazure_Fabric_to_Fabric_discovery_df` data flow
...
* `DATASET` - This is used to identify data that belongs to this pipeline in the metadata store (default `FABRIC`)
