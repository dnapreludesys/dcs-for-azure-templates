# dcsazure_Fabric_to_Fabric_mask_pl
## Delphix Compliance Services (DCS) for Azure - Fabric_to_Fabric Masking Pipeline

This pipeline will perform masking of your Fabric warehouse.
...
`Fabric warehouse` (source) - Linked service associated with Fabric source data. This will be used for the
following steps:
* dcsazure_Fabric_to_Fabric_filter_test_utility_df/Source (dataFlow)
* dcsazure_Fabric_to_Fabric_unfiltered_mask_df/Source (dataFlow)
* dcsazure_Fabric_to_Fabric_filtered_mask_df/Source (dataFlow)
* dcsazure_Fabric_to_Fabric_mask_source_ds (Azure SQL Database dataset)
* dcsazure_Fabric_to_Fabric_copy_df/SourceData (dataFlow)

`Fabric warehouse` (sink) - Linked service associated with Fabric sink data. This will be used for the
following steps:
* Truncate Selected Table (Script activity)
* dcsazure_Fabric_to_Fabric_filter_test_utility_df/Sink (dataFlow)
* dcsazure_Fabric_to_Fabric_unfiltered_mask_df/Sink (dataFlow)
* dcsazure_Fabric_to_Fabric_filtered_mask_df/Sink (dataFlow)
* dcsazure_Fabric_to_Fabric_mask_sink_ds (Azure SQL Database dataset)
* dcsazure_Fabric_to_Fabric_copy_df/SinkData (dataFlow)
...
* dcsazure_Fabric_to_Fabric_mask_metadata_ds (Azure SQL Database dataset)
* dcsazure_Fabric_to_Fabric_unfiltered_mask_params_df/Ruleset (dataFlow)
* dcsazure_Fabric_to_Fabric_unfiltered_mask_params_df/TypeMapping (dataFlow)
* dcsazure_Fabric_to_Fabric_filtered_params_df/Ruleset (dataFlow)
* dcsazure_Fabric_to_Fabric_filtered_params_df/TypeMapping (dataFlow)

`REST` (DCS for Azure) - Linked service associated with calling DCS for Azure. This will be used for the following
steps:
* dcsazure_Fabric_to_Fabric_unfiltered_mask_df (dataFlow)
* dcsazure_Fabric_to_Fabric_filtered_mask_df (dataFlow)
  ...
  * Filter If Copy Unmask Enabled. This is done by applying a filter based on the value of `P_COPY_UNMASKED_TABLES`
    * For Each Table With No Masking. Provided we have any rows left after applying the filter
      ...
      * If Copy Via Dataflow - based on the value of `P_COPY_USE_DATAFLOW`
        * If the data flow is to be used for copy, then call `dcsazure_Fabric_to_Fabric_copy_df`
  ...
  * For Each Table To Mask
    * Check if the table must be masked with a filter condition
      * If no filter needs to be applied:
        * Call the `dcsazure_Fabric_to_Fabric_unfiltered_mask_params_df` data flow to generate masking parameters
        * Call the `dcsazure_Fabric_to_Fabric_unfiltered_mask_df` data flow, passing in parameters as generated by the generate masking parameters dataflow
        * Update the mapped status based on the success of this dataflow, and fail accordingly
      * If a filter needs to be applied:
        * Call the `dcsazure_Fabric_to_Fabric_filtered_mask_params_df` data flow to generate masking parameters using the filter alias
        * Call the `dcsazure_Fabric_to_Fabric_filtered_mask_df` data flow, passing in parameters as generated by the generate masking parameters dataflow and the filter as determined by the output of For Each Table To Mask
        * Update the mapped status based on the success of this dataflow, and fail accordingly
...
* `DATASET` - This is the way this data set is referred to in the metadata store (default `FABRIC`)
